...
class EOSGetEventCommand(session: Session): Command(session) {
    ...
    override fun encodeCommand(byteBuffer: ByteBuffer) {
        try {
            encodeCommand(byteBuffer, PtpConstants.Operation.EosEventCheck.toShort())
    ...
    companion object {
        const val TAG = "EOSGetEventCommand"
    }
}