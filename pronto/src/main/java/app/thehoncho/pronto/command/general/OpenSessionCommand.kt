...
class OpenSessionCommand(session: Session): Command(session) {
    ...
    override fun encodeCommand(byteBuffer: ByteBuffer) {
        try {
            encodeCommand(byteBuffer, PtpConstants.Operation.OpenSession, 1)
    ...
    companion object {
        private const val TAG = "OpenSessionCommand"
    }
}